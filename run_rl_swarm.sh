...
if [ "$CONNECT_TO_TESTNET" = true ]; then
    ...
    echo_green ">> Setting up required directories and files..."
    # Create the required directory structure
    mkdir -p "$ROOT/modal-login/temp-data/"
    
    # Copy backup files if they exist
    if [ -f "/root/rl-swarm-backup/swarm.pem" ]; then
        cp /root/rl-swarm-backup/swarm.pem "$ROOT/"
        echo "Copied swarm.pem from backup"
    fi
    
    if [ -f "/root/rl-swarm-backup/userApiKey.json" ]; then
        cp /root/rl-swarm-backup/userApiKey.json "$ROOT/modal-login/temp-data/"
        echo "Copied userApiKey.json from backup"
    fi
    
    if [ -f "/root/rl-swarm-backup/userData.json" ]; then
        cp /root/rl-swarm-backup/userData.json "$ROOT/modal-login/temp-data/"
        echo "Copied userData.json from backup"
    fi
...
if [ -n "${HF_TOKEN}" ]; then # Check if HF_TOKEN is already set and use if so. Else give user a prompt to choose.
    ...
    echo_green ">> Automatically selecting 'N' for Hugging Face Hub - models will NOT be pushed to HF Hub"
    HUGGINGFACE_ACCESS_TOKEN="None"
fi

echo_green ">> Automatically using default model (pressing Enter)"
MODEL_NAME=""
